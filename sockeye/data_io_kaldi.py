...
class RawParallelDatasetLoader:
    ...
    def __init__(self,
                 ...
                 pad_id: int,
                 dtype: str = 'float32',
                 source_embed_size: Optional[int] = 0 ) -> None:
        self.buckets = buckets
        self.eos_id = eos_id
        self.pad_id = pad_id
        self.dtype = dtype
...
def get_validation_data_iter(data_loader: RawParallelDatasetLoader,
    ...
    source_embed_size=len(vocab_source)
...
def read_content(path: str, itype: str, limit: Optional[int] = None) -> Iterator[List[str]]:
    """
    ...
    """
    if itype == "scp":
        for key,mat in kaldi_io.read_mat_scp(path):
            yield mat
    elif itype == "lab":
        for key,mat in kaldi_io.read_vec_int_ark(path):
            yield mat+4 #used to make it start from 1: in decoder, use # of !=0 to count length; <pad> <unk> <s> </s>, so add 4
    else:
...
class SequenceReader(Iterator):
    ...
    def __iter__(self):
        ...
        self._iter = read_content(self.path, self.itype, self.limit)
...
class ParallelSampleIter(BaseParallelSampleIter):
    ...
    def __init__(self,
                 ...
                 source_embed_size : Optional[int] = 0) -> None:
        super().__init__(buckets, batch_size, bucket_batch_sizes,
                         source_data_name, target_data_name, label_name, dtype, source_embed_size = source_embed_size)
